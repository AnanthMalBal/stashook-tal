...
-- Dumping data for table data_process.access_privilege: ~59 rows (approximately)
...
-- Dumping data for table data_process.access_privilege_role: ~65 rows (approximately)
...
DELETE FROM `application_schedulers`;
...
WHILE @currentDate <= @lastDate DO 
   ...
   -- Default Colour Scheme Normal - White	
	...
	SELECT `status` INTO @LeaveType FROM usersleavemanagement WHERE employeeId = employeePrimaryId AND DATE(@currentDate) BETWEEN fromDate AND toDate;
	...
	SELECT UHC.symbol INTO @LeaveType FROM usersholidayscalendar UHC WHERE UHC.startDate = @currentDate;
...
INSERT INTO `menu` (`menuId`, `parentId`, `menuName`, `cssClassIcon`, `actionURL`, `cssClass`, `level`, `alignment`, `toggle`, `translate`, `bullet`, `title`) VALUES
	('ApplyLeave', 'LeaveManagement', 'Apply Leave', 'icon-tag', '/leave-management/lms', 'title\r\n', 'FA', 'left', 'click', 'MENU.LEAVEHISTORY', '', 'My Leave'),
...
DELETE FROM `producers_attachments`;
...
DELETE FROM `producers_collabrate`;
...
-- Dumping data for table data_process.producers_property: ~1 rows (approximately)
DELETE FROM `producers_property`;
...
DELETE FROM `timesheet_process_project`;
...
-- Dumping data for table data_process.tix_queue: ~15 rows (approximately)
DELETE FROM `tix_queue`;
...
DELETE FROM `tix_queue_status_reason`;
...
-- Dumping data for table data_process.tix_user_department: ~5 rows (approximately)
DELETE FROM `tix_user_department`;
...
-- Dumping data for table data_process.tix_user_division: ~8 rows (approximately)
DELETE FROM `tix_user_division`;
...
-- Dumping data for table data_process.users_assigned_tasks: ~1 rows (approximately)
DELETE FROM `users_assigned_tasks`;
...
-- Dumping data for table data_process.users_attendance: ~0 rows (approximately)
DELETE FROM `users_attendance`;
...
DELETE FROM `users_holidays_calendar`;
...
-- Dumping data for table data_process.users_leave_management: ~2 rows (approximately)
DELETE FROM `users_leave_management`;
...
-- Dumping data for table data_process.users_log: ~45 rows (approximately)
...
DELETE FROM `users_resource_project`;
...
-- Dumping data for table data_process.users_roles: ~21 rows (approximately)
...
-- Dumping data for table data_process.users_timesheet: ~0 rows (approximately)
DELETE FROM `users_timesheet`;
